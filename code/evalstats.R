...
evalStats<-function(true,estimated,tol){
  ...
  if(all(is.na(estimated))){ #if there are no disturbances . . . 
    ...
    for(ii in 1:nrow(estimated)){
      detect.ii<-ifelse(estimated$dist.date[ii] > true["dday"] - tol & 
                          estimated$dist.date[ii] < true["rday"],1,0)
      delta.dist.ii<-true["dday"]-estimated$dist.date[ii]
      delta.recov.ii<-true["rday"]-estimated$recov.date[ii]
      delta.tdiff.ii<-true["recovtime"]-estimated$tdiff[ii]
